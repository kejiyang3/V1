...
#include "app_log.h"
//extern void APP_USB_LOG(const char *format, ...);
...
static void ECG_USB_DumpFile_Blocking(void)
{
    ...
    APP_USB_LOG("[USB_DUMP] state=%d sd_file_closed=%d file=%s\r\n",
    ...
    if (g_ecg_rec.state == ECG_REC_RECORDING ||
        ...
        APP_USB_LOG("[USB_DUMP][WARN] ECG is still recording. Stop first.\r\n");
        return;
    }

    APP_USB_LOG("[USB_DUMP] begin dump file: %s\r\n", g_ecg_rec.file_name);
    ...
    if (res != FR_OK) {
        APP_USB_LOG("[USB_DUMP][ERR] f_mount failed: %d\r\n", res);
        goto release;
    }

    res = f_open(&file, g_ecg_rec.file_name, FA_READ);
    if (res != FR_OK) {
        APP_USB_LOG("[USB_DUMP][ERR] f_open failed: %d\r\n", res);
        goto release;
    }
    ...
    while (1) {
        ...
        if (res != FR_OK) {
            APP_USB_LOG("[USB_DUMP][ERR] f_read failed: %d\r\n", res);
            break;
        }
        ...
        if (ECG_USB_DUMP_MAX_BYTES != 0 &&
            ...
            if (remain > 0) {
                ...
                if (ret != USBD_OK) {
                    APP_USB_LOG("[USB_DUMP][ERR] CDC send failed: %u\r\n", ret);
                } else {
                    total_sent += remain;
                }
            }
...
release:
    ...
    if (partial) {
        APP_USB_LOG("[USB_DUMP] partial dump finished, sent_bytes=%lu (max %lu)\r\n",
                        total_sent, (uint32_t)ECG_USB_DUMP_MAX_BYTES);
    } else {
        APP_USB_LOG("[USB_DUMP] finished, sent_bytes=%lu\r\n", total_sent);
    }
}
...
void StartTask_ECG_USBDump(void *argument)
{
    ...
    APP_USB_LOG("[USB_DUMP] task entered\r\n");

    for (;;) {
        if (s_usb_dump_request) {
            s_usb_dump_request = 0;
            APP_USB_LOG("[USB_DUMP] request received\r\n");

            /* SD 文件未关闭时等待 */
            if (!g_ecg_rec.sd_file_closed) {
                APP_USB_LOG("[USB_DUMP][WARN] SD file not closed, waiting\r\n");
