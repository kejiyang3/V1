...
#include "cmsis_os.h"
#include "stm32l4xx.h"
/* RX DMA is now configured (DMA2_Channel4), both TX and RX use DMA */
extern DMA_HandleTypeDef hdma_sdmmc1_tx;
extern DMA_HandleTypeDef hdma_sdmmc1_rx;
extern SD_HandleTypeDef hsd1;
#define SD_LOG(...)              usb_printf(__VA_ARGS__)
...
extern volatile uint8_t g_rtos_started; /* defined in main.c, set=1 after osKernelStart() */
...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  /* [CRITICAL FIX] 强制恢复为 Polling 模式读取，防止 RX DMA 缺失导致瞬间失败 */
  (void)lun;
...
void BSP_SD_AbortCallback(void)
{
  ...
  osMessagePut(SDQueueID, READ_CPLT_MSG, 0);
#else
  const uint16_t wmsg = WRITE_CPLT_MSG;
  const uint16_t rmsg = READ_CPLT_MSG;
  osMessageQueuePut(SDQueueID, (const void *)&wmsg, 0, 0);
  osMessageQueuePut(SDQueueID, (const void *)&rmsg, 0, 0);
#endif
}
