/**
 * @file app_lvgl.c
 * @brief LVGL ECG External Test UI — 两页: Main / Diagnostic
 */
#include "app_lvgl.h"
#include "lvgl.h"
...
static lv_obj_t *ui_label_queue;
...
static void ui_update_cb(lv_timer_t *timer)
{
    ...
    if (s_page == 0) {
        ...
        lv_label_set_text_fmt(ui_label_fifo, "FIFO: %lu", g_ecg_rec.fifo_sample_count);
    ...
    if (s_page == 1) {
        ...
        lv_label_set_text_fmt(ui_label_queue,      "Q: %lu/%lu",
                              ECG_SDLogger_GetQueueCount(),
                              ECG_SDLogger_GetQueueCapacity());
    }
}
...
void App_LVGL_TestUI(void)
{
    ...
    lv_label_set_text(ui_label_title, "ECG EXT TEST");
    ...
    lv_obj_align(ui_label_title, LV_ALIGN_TOP_MID, 0, 6);
    ...
    lv_obj_align(ui_label_state, LV_ALIGN_TOP_MID, 0, 24);
    ...
    lv_obj_align(ui_label_mode, LV_ALIGN_TOP_LEFT, 10, 42);

    lv_obj_t *ui_btn_mode = lv_btn_create(lv_scr_act());
    lv_obj_set_size(ui_btn_mode, 34, 18);
    lv_obj_align(ui_btn_mode, LV_ALIGN_TOP_LEFT, 110, 42);
    ...
    lv_obj_align(ui_label_rate, LV_ALIGN_TOP_LEFT, 10, 60);

    /* File */
    ui_label_file = lv_label_create(lv_scr_act());
    lv_label_set_text(ui_label_file, "File: ecg_open_000.csv");
    lv_obj_set_style_text_color(ui_label_file, lv_color_hex(0xFFFFFF), 0);
    lv_obj_align(ui_label_file, LV_ALIGN_TOP_LEFT, 10, 74);

    /* FIFO */
    ui_label_fifo = lv_label_create(lv_scr_act());
    lv_label_set_text(ui_label_fifo, "FIFO: 0");
    lv_obj_set_style_text_color(ui_label_fifo, lv_color_hex(0x88CC88), 0);
    lv_obj_align(ui_label_fifo, LV_ALIGN_TOP_LEFT, 10, 90);
    ...
    lv_obj_align(ui_label_samples, LV_ALIGN_TOP_LEFT, 10, 104);
    ...
    lv_obj_align(ui_label_drop, LV_ALIGN_TOP_LEFT, 10, 118);
    ...
    lv_obj_align(ui_label_written, LV_ALIGN_TOP_LEFT, 10, 132);

    /* Start/Stop */
    ui_btn_start = lv_btn_create(lv_scr_act());
    lv_obj_set_size(ui_btn_start, 160, 40);
    lv_obj_align(ui_btn_start, LV_ALIGN_BOTTOM_MID, 0, -56);
    lv_obj_set_style_bg_color(ui_btn_start, lv_color_hex(0x33AA33), 0);
    ...
    lv_obj_set_size(ui_btn_info, 160, 32);
    ...
    ui_label_queue = lv_label_create(lv_scr_act());
    lv_label_set_text(ui_label_queue, "Q: 0/1024");
    lv_obj_set_style_text_color(ui_label_queue, lv_color_hex(0x44AAFF), 0);
    lv_obj_align(ui_label_queue, LV_ALIGN_TOP_LEFT, 10, y); y += dy;

    /* Hide page 2 by default */
    lv_obj_t *hide_init[] = {
        ui_label_title2, ui_label_status_reg, ui_label_pll_seen,
        ui_label_pll_edge, ui_label_eovf, ui_label_queue
    };
